...
function _fzf_git_add() {
	...
	git add $(git diff $@ --name-only | fzf -m --ansi --preview "$preview" | xargs echo)
} 
...
function _fzf_git_checkout() {
	local preview="git diff $@ --color=always -- {-1}"
	git checkout $(git diff $@ --name-only | fzf -m --ansi --preview "$preview" | xargs echo)
} 

alias gpush="_git_push_and_open_the_site"
function _git_push_and_open_the_site() {
	git push origin head
	local tmp=$(git remote -v | head -n1 | awk -F'git@' '{print $2}' | awk -F' ' '{print $1}')
	open "https://$tmp"
}
